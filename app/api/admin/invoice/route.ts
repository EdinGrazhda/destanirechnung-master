...
export const POST = async (req: any, res: NextResponse) => {
    ...
    try{
        ...
        if (!company || company.length <= 0 ||
          ...
          return NextResponse.json({ message: "Please fill the form!" }, { status: 400 });
        }
        ...
        const foundWithCustomIDInvoice = await Invoice.findOne({ textId: customID });
        console.log(foundWithCustomIDInvoice);
        if (foundWithCustomIDInvoice !== null && foundWithCustomIDInvoice !== undefined) {
          return NextResponse.json({ message: "An invoice with this ID already exists!" }, { status: 409 });
        }
        ...
        return NextResponse.json({ Message: "Success", newSavedInvoice: newSavedInvoice }, { status: 200 });
    }
    catch (e) {
        console.log(e);
...
export const GET = async (req: NextRequest, res: NextResponse) => {
  ...
  if (invoiceStatus === "dashboard") {
    ...
    const page = Number.isInteger(invoicesPage) && invoicesPage > 0 ? invoicesPage : 1;
    ...
    const foundInvoices = await Invoice.find({})
      ...
      .limit(pageSize);
    return NextResponse.json({ message: "Success", foundInvoices: foundInvoices }, { status: 200 });
  } else if (invoiceStatus === "paid" || invoiceStatus === "all") {
    const filteringMonthYearOptionParam = searchParams.get("filteringMonthYearOptionParam");
    console.log(filteringMonthYearOptionParam);

    let dateFilter = {};

    if (filteringMonthYearOptionParam && filteringMonthYearOptionParam !== "all") {
    ...
    return NextResponse.json({ message: "Success", foundInvoices }, { status: 200 });
  } else {
    const foundInvoices = await Invoice.find(invoiceStatus === "all" ? {} : {status: invoiceStatus});
    return NextResponse.json({ message: "Success", foundInvoices: foundInvoices }, { status: 200 });
  }
